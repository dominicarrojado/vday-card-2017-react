  ...
  handleSubmit(event) {
    ...
    if (isStringEmpty(to) || isStringEmpty(from) || isStringEmpty(message)) {
      return self.setState({
        error:
          'Do not rush love! Please fill out everything before publishing.',
      });
    }
    ...
    return (
      <div id="containerMain" className="container-main overflow">
        <div id="right" className="right">
              ...
              {getCovers().map((cover, index) => {
                return (
                  <li
                    ...
                    className={coverActive === cover ? 'active' : ''}
                  >
                    <div
                      style={{
                        backgroundImage: `url('/images/icons/icon-cover-${cover}.png')`,
                      }}
                    />
                  </li>
                );
          ...
          <div className="date">{CardCreate.currentDate}</div>
          <form onSubmit={handleSubmit}>
            ...
            <div className="message">
              ...
              />
              <button
                onClick={() => Modal.show(<ModalSuggestion />)}
                type="button"
              >
            ...
            <div className="text-center">
              <button
                type="submit"
                className={`btn-${getButtonColor(coverActive)} btn`}
              >
                Publish
              </button>
            </div>
        ...
        <div className="left">
          <div
            onClick={CardCreate.maximizeCover}
            id="cover"
            className={`bg-${coverActive} cover`}
          >
            {getCovers().map((cover, index) => {
              return (
                <img
                  key={index}
                  src={`/images/cover-${cover}.gif`}
                  className={coverActive === cover ? 'show' : ''}
                />
              );
            })}
          </div>
          <button
            onClick={CardCreate.minimizeCover}
            id="btnClose"
            className="btn-close"
          >
            &times;
